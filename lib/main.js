...
// Open source node.js module for accessing Sincerely's Ship API:
// <https://dev.sincerely.com/docs>
...
module.exports = function(appKey, https) {
  ...
  function prepareRequest(method, path, data, cb) {
    ...
    if ('sender' in data)
      data.sender = JSON.stringify(data.sender);
    ...
    requestData = 'appkey=' + appKey + '&' + requestData;
  ...
  return {
    ...
    create: function(data, cb) {
      post('/shiplib/create', data, cb);
    },

    // # Upload
    upload: function(data, cb) {
      post('/shiplib/upload', data, cb);
    },

    // # Debug
    debug: function(data, cb) {
      post('/shiplib/debug', data, cb);
    },

    // # Cancel
    cancel: function(data, cb) {
      post('/shiplib/cancel', data, cb);
    }
